export default function BackLogDropHandler(
  e,
  orderIssuesMutation,
  updateIssueMutation,
  dispatch,
  ...
  if (source.droppableId === destination.droppableId) {
    ...
    list = list.map((item, index) => ({ ...item, logIndex: index }));

    let updatedIssues = { ...issues };
    updatedIssues[destination.droppableId] = list;
    dispatch(setIssues(sortArraysByIndex(updatedIssues)));
    ...
    updateIssueMutation.mutate({
      ...
      load: {
        ...
        updatedData: {
          log: logs.filter((e) => e.label == destination.droppableId)[0]._id,
        },
      },
    });
    orderIssuesMutation.mutate({
      route: "issue/reorder",
      method: "POST",
      load: { newOrder: sourceList },
    });
    orderIssuesMutation.mutate({
      route: "issue/reorder",
      method: "POST",
      load: { newOrder: destinationList },
    });
  }
}
...
export const organize = (data) => {
  ...
  data.forEach((item) => {
    const { log } = item;
    if (!groupedissues[log.label]) {
      groupedissues[log.label] = [];
    }

    groupedissues[log.label].push(item);
  });
